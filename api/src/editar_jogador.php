<?php
// editar_jogador.php - Editar jogador existente
...
// Buscar jogador
$stmt = $pdo->prepare("SELECT * FROM jogador WHERE id = :id");
$stmt->execute([':id' => $id]);
$jogador = $stmt->fetch();
if (!$jogador) { header("Location: jogadores.php?msg=erro"); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE jogador SET nome = :nome, posicao = :posicao WHERE id = :id");
            $stmt->execute([':nome' => $nome, ':posicao' => $posicao, ':id' => $id]);
            header("Location: jogadores.php?msg=editado");
            exit;
...
// Valores a exibir no form
$nomeFill    = $_POST['nome']    ?? $jogador['nome'];
$posicaoFill = $_POST['posicao'] ?? $jogador['posicao'];
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-ativo { color: var(--azul) !important; border-color: var(--azul) !important; }

        .container { max-width: 520px; margin: 50px auto; padding: 0 20px; }
        ...
        .subtitulo { font-size: 0.8rem; color: var(--texto-suave); margin-bottom: 28px; }
        ...
        .btn-azul { background: var(--azul); color: var(--preto); }
...
<div class="container">
    <div class="card">
        ...
        <p class="subtitulo">ID: #<?= $id ?></p>

        <?php if ($erro): ?>
            <div class="alerta-erro">✘ <?= htmlspecialchars($erro) ?></div>
        <?php endif; ?>

        <form method="POST" action="editar_jogador.php?id=<?= $id ?>">
            <label for="nome">Nome do Jogador</label>
            <input type="text" id="nome" name="nome"
                   value="<?= htmlspecialchars($nomeFill) ?>"
                   required>
